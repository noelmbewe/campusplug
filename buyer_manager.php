...
class BuyerManager {
    ...
    public function getProducts($filters = []) {
        try {
            $query = "SELECT p.product_id, p.name, p.description, p.image_url, p.price, p.listing_type, c.name AS category
                      FROM products p
    ...
    public function getCart() {
        try {
            $stmt = $this->pdo->prepare("SELECT ci.cart_item_id, ci.product_id, ci.quantity, ci.listing_type, p.name, p.price, p.image_url
                                         FROM cart_items ci
        ...
        try {
            ...
            foreach ($cart_items as $item) {
                $stmt = $this->pdo->prepare("INSERT INTO order_items (order_id, product_id, quantity, price, listing_type) 
                                             VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$order_id, $item['product_id'], $item['quantity'], $item['price'], $item['listing_type']]);
            }
    ...
    private function processPayment($payment_details, $amount) {
        ...
        if (empty($payment_details['card_number']) || strlen($payment_details['card_number']) < 16) {
            return false;
        }
    ...
    public function getOrders() {
        try {
            $stmt = $this->pdo->prepare("SELECT o.order_id, o.total_amount, o.status, o.created_at, o.shipping_address
                                         FROM orders o
                                         WHERE o.user_id = ?");
            $stmt->execute([$this->user_id]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getOrderItems($order_id) {
        try {
            $stmt = $this->pdo->prepare("SELECT oi.product_id, oi.quantity, oi.price, oi.listing_type, p.name, p.image_url
                                         FROM order_items oi
